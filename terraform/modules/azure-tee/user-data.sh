#!/bin/bash
# FCM TEE Agent Server — Azure cloud-init bootstrap
set -euo pipefail

echo "=== FCM TEE Agent Setup: ${agent_id} (${agent_type}) ==="
...
if [ -e /dev/sgx_enclave ]; then
  ...
  chmod 666 /dev/sgx_enclave /dev/sgx_provision 2>/dev/null || true
else
  echo "SGX not available — running without hardware TEE"
fi
...
echo "=== FCM TEE Agent ${agent_id} ready ==="
