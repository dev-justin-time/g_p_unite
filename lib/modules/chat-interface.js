...
class ChatInterface {
    ...
    async processMessage(message) {
        ...
        const sanitized = trimmed.replace(/[<>"'`]/g, '').slice(0, 1024);
        ...
        const recentCommands = this.history.filter(h => h.role === 'user' && Date.now() - new Date(h.time).getTime() < 60000);
        if (recentCommands.length > 60) {
            return { text: "Rate limit: max 60 commands per minute.", type: "error" };
        }

        // Route to handler
        const commandDef = COMMANDS[cmd];
        if (!commandDef) {
            // Try natural language understanding (use sanitized input)
            const nlResponse = await this._naturalLanguageHandler(sanitized);
    ...
    async handleGrant(args) {
        ...
        const caller = this.master.adminAddress;
        if (!caller || !this.master.permissionManager.hasPermission(caller, 'system:config')) {
            return { text: "Permission denied: requires system:config to grant permissions.", type: "error" };
        }
    ...
    async handleBan(args) {
        ...
        const caller = this.master.adminAddress;
        if (!caller || !this.master.permissionManager.hasPermission(caller, 'system:config')) {
            return { text: "Permission denied: requires system:config to ban users.", type: "error" };
        }
