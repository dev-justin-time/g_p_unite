/**
 * G P Unite — Agent Data Definitions
 * All 18 agents with logic rules, source code, and live tick functions
 */
...
const AGENTS = [
  ...
  return least_loaded(req);
    ...
    tick(v) { v.tps = 4800 + Math.floor(Math.random() * 200); v.queue = Math.max(0, v.queue + Math.floor(Math.random() * 3) - 1); }
  },
  {
  ...
  return mpc_sum(noised, threshold=t+1);
    ...
    tick(v) { v.rounds = v.rounds + (Math.random() > 0.9 ? 1 : 0); v.accuracy = (94 + Math.random() * 1).toFixed(1) + '%'; }
  },
  {
  ...
  distribute_fcm(address, reward);
    ...
    tick(v) { v.tasks = v.tasks + (Math.random() > 0.7 ? 1 : 0); }
  },
  ...
  release_funds(escrow.client, amount);
    ...
    tick(v) { v.milestones = v.milestones + (Math.random() > 0.8 ? 1 : 0); }
  },
  {
...
const NAV_ITEMS = ['onboarding', 'dashboard', 'agents', 'marketplace', 'staking', 'escrow', 'governance', 'reputation', 'chat', 'resources', 'settings', 'admin'];
...
const RBAC_PERMISSIONS = {
  admin: { label: 'Admin', icon: '🛡', cssClass: 'admin', nav: NAV_ITEMS, actions: { stake: true, unstake: true, claim_task: true, create_proposal: true, vote: true, approve_milestone: true, send_chat: true, manage_roles: true, pause_contracts: true, emergency_actions: true, edit_settings: true, launch_node: true, configure_multi_sig: true } },
  operator: { label: 'Operator', icon: '⚙', cssClass: 'operator', nav: ['onboarding', 'dashboard', 'agents', 'marketplace', 'staking', 'escrow', 'governance', 'reputation', 'chat', 'resources', 'settings'], actions: { stake: true, unstake: true, claim_task: true, create_proposal: true, vote: true, approve_milestone: true, send_chat: true, manage_roles: false, pause_contracts: false, emergency_actions: false, edit_settings: true, launch_node: true, configure_multi_sig: false } },
  viewer: { label: 'Viewer', icon: '👁', cssClass: 'viewer', nav: ['dashboard', 'agents', 'marketplace', 'governance', 'reputation', 'resources'], actions: { stake: false, unstake: false, claim_task: false, create_proposal: false, vote: false, approve_milestone: false, send_chat: false, manage_roles: false, pause_contracts: false, emergency_actions: false, edit_settings: false, launch_node: false, configure_multi_sig: false } }
};
